...
const trampoline = next => value => {
  while (is.type.function(next)) { [ next, value ] = apply(next)(value) }
  return value
}
...
const ᐅon = path => action => ᐅ([ get.at_path(path), action ])
...
const ᐅwherein = path => test => update => ᐅin(path)(ᐅwhen(test)(update))
const ᐅwhereon = path => test => action => ᐅon(path)(ᐅwhen(test)(action))
// chainable effects
...
export {
  ...
  ᐅif      as pipe_if,
  ᐅwhen    as pipe_when,
  ᐅwhereon as pipe_whereon,
  ᐅwherein as pipe_wherein,
  // chainable effects
  ᐅeffect,
  ᐅlog,
...
export function test (harness) {
  ...
  return harness.suite(`@prettybad/util: it's worse than underscore`, [
    ...
    t => t.suite('pipes (ᐅ)', {
      ...
      'ᐅon: acts on a value in an object at a path; returns value': [
        t => {
          const ab5 = { a: { b: 5 } }
          return true
            && t.eq(ᐅon([ 'a', 'b' ])(v => v + 1)(ab5))(6)
        },
      ],
      'ᐅwhereon: guards an action on the value in an object at a path': [
        t => {
          const ab5 = { a: { b: 5 } }
          const ab8 = { a: { b: 8 } }
          return true
            && t.eq(ᐅwhereon([ 'a', 'b' ])(is(5))(v => v + 1)(ab5))(6)
            && t.eq(ᐅwhereon([ 'a', 'b' ])(is(5))(v => v + 1)(ab8))(8)
        },
      ],
      'ᐅwherein: guards an action on an object in a value at a path': [
        t => {
          const ab5 = { a: { b: 5 } }
          const ab6 = { a: { b: 6 } }
          const ab8 = { a: { b: 8 } }
          return true
            && t.eq(ᐅwherein([ 'a', 'b' ])(is(5))(v => v + 1)(ab5))(ab6)
            && t.eq(ᐅwherein([ 'a', 'b' ])(is(5))(v => v + 1)(ab8))(ab8)
        },
      ],
    }),
    t => t.suite('functions', {

...
const time = thunk => {
  ...
  return {
    ...
    summary: (() => {
      const s = diff / BigInt(1e9)
      let rem = diff - (s * BigInt(1e9))
    ...
    in: {
      ns: diff,
      get s  () { return this.ns / BigInt(1e9) },
      get ms () { return this.ns / BigInt(1e6) },
      get us () { return this.ns / BigInt(1e3) },
    },
  }
}
...
const or_none = predicate => fn => ᐅif(predicate)(fn)(_ => None)
 ...
 *
 * INDEXED and UNINDEXED shuttles: a fallible normally returns a tuple of
 * (succeeded, posed-result). This is called the "shuttle," analogous to a
 * shuttle in a loom: it passes through computations to build up a result.
 * This normal case is the UNINDEXED shuttle. However, in any composition
 * of fallibles written using `fold`, the shuttle is transparently
 * extended into an INDEXED shuttle, making it possible to identify which
 * fallible failed in a large composition. An INDEXED shuttle is then
 * (succeeded, posed-result, index). Whether a shuttle is indexed is a
 * simple test of the tuple length. This index can be used to recover the
 * specific failed fallible in any failed composition written as a fold.
 */
...
function fallible_fold (folder) {
  return initial => fold_indexed(index => value => {
    ...
    return ᐅif(get(0))(thread_shuttle)(_ => fold.break)
  })([ true, initial, -1 ])
}
...
const ᐅat = path => action => ᐅ([ get.at_path(path), action ])
...
// TODO(jordan): deprecate reflex in favor of is.{type,instance}
export const reflex = { type, instance }
...
export function test (harness) {
  ...
  return harness.suite(`@prettybad/util: it's worse than underscore`, [
    ...
    t => t.suite('pipes (ᐅ)', {
      ...
      'ᐅon: acts on a key in an object; returns value': [
        t => {
          const ab5 = { a: { b: 5 } }
          return true
      ...
      'ᐅat: acts on a value in an object at a path; returns value': [
        t => {
          const ab5 = { a: { b: 5 } }
          const ab8 = { a: { b: 8 } }
          return true
      ...
      'ᐅin: acts on an object in a value at a path; returns object': [
        t => {
          const ab5 = { a: { b: 5 } }
          const ab6 = { a: { b: 6 } }
          const ab8 = { a: { b: 8 } }
    ...
    t => t.suite('fallibles (ƒ)', {
      ...
      'ƒ.await: await a promise as a promise<fallible>': [
        t => { /* TODO */ },
      ],
        ...
        'at_path': {
          'value': [
            t => {
              const abc5 = { 'a': { 'b': { 'c': 5 } } }
              return true
                  && t.eq(get.in(abc5).at_path.value(['a','b','c']))(5)
            },
          ],
          'entry': [
            t => { /* TODO */ },
    ...
    t => t.suite('short-circuitable looping primitives', {
      ...
      '{fold,over}: perform a fold over an array of values; can break': [
        t => {
          const sum = v => a => a + v
          const break_gt_4 = v => ᐅif(a => a > 4)(_ => fold.break)(sum(v))
          return true

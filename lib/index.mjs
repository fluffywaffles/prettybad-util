...
const ᐅwhereon = path => predicate => action  => {
  return ᐅ([ get.at_path(path), ᐅwhen(predicate)(action) ])
}
const ᐅwherein = path => predicate => updater => {
  const at_path = fn => ᐅ([ get.at_path(path), fn ])
  const action  = ƒ.fatalize(update_path(path)(updater))
  return ᐅwhen(at_path(predicate))(action)
}

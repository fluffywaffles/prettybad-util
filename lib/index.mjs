    ...
    const thread_shuttle = index => item => ᐅ([
      ...
      ᐅdo([
        ...
        ([ succeeded2, result2, carry2 ]) => ([ _, carry1 ]) => ([
          succeeded2,
          result2,
          merge_properties([ carry1, carry2, { index } ]),
        ])
      ])
    ])
    return ᐅif(/* succeeded */get(0))(thread_shuttle(index)(item))(_ => fold.break)
...
const drop_end = taker(take => n => ᐅif(ᐅ([ len, l => l > n ]))(ᐅ([
  ᐅwhen(_ => is(-0)(n))(_ => fold.break),
...
const get = js.assign({
  ...
  at_path: js.assign({
    ...
    values      : p => keys => ᐅat_path(get_values(keys))(p),
    entries     : p => keys => ᐅat_path(get_entries(keys))(p),
    properties  : p => keys => ᐅat_path(get_properties(keys))(p),
    descriptors : p => keys => ᐅat_path(get_descriptors(keys))(p),
      ...
      t => t.suite(`for_keys`, {
        ...
        'properties': [
          ...
          t => t.eq(get.for_keys.properties([ 'z' ])({ x: { y: 123 }, k: true }))
                   ([['z', None]]),
          t => t.eq(get.for_keys.properties([ 'x', 'k' ])(from_descriptors({ x: d.nothing({ v: 123 }) })))
                   ([
